...
const createBoardRow = (numbers: number[], rowId: number = 0): utils.Cell[] =>
  numbers.map((number, index) =>
  utils.generateCell(utils.generateCoordinate(rowId, index), number)
  );
...
describe("sudoku/utils", () => {
  describe("doesValueExistInRow", () => {
    const board = [
      createBoardRow([1, 3, 0, 0, 0, 5, undefined, 0, 0]),
  ...
  describe("doesValueExistInColumn", () => {
    const board = [
      createBoardRow([1, 3, undefined, 8, 2, 5, 4, 6, 7]),
  ...
  describe('createDifficultyMap',() => {
    it('creates a map to track how many occurrences of a number should be displayed in the board', () => {
      const res = utils.createNumberDifficultyMap([2,4,6], utils.VALID_NUMBERS)
  ...
  describe('createGame',() => {
    ...
    it('creates a "EASY" difficulty game', () => {
      const numberDifficultyMap = utils.createNumberDifficultyMap(utils.difficulty.EASY, shuffle(utils.VALID_NUMBERS))
