import { shuffle, isEqual, isEmpty } from 'lodash'
...
import { generateBoard, createGame, difficulty, createNumberDifficultyMap, countAutoGeneratedCells, getConflicts } from './utils'
import { getCellByCoordinate } from './board'
...
export const setCellCandidate = (state: State, number: number) => {
  ...
  if (!selectedCell || getCellByCoordinate(selectedCell, game).value) {
    return state
  }
  clearConflictingCells(game, conflictingCells)
...
export const getCandidateConflictingCells = (candidate: number, selectedCell: Coordinate, game: Cell[][]): Coordinate[] => {
  const candidateConflicts = getConflicts(candidate, selectedCell, game)
...
export const setCandidate = (game: Cell[][], selectedCell: Coordinate, number: number) => {
  const {x, y} = selectedCell
  const cell = game[x][y];
