...
*/
export const createNumberDifficultyMap = (difficulty: number[], numbers: number[]): NumberMap => {
  return difficulty.reduce(
    (acc: NumberMap, cur: number, index) => {
      const num = cur
      acc[numbers[index]] = { count: random(num - 1, num), coordinates: [], candidates: [] }
      return acc
    },
    {}
  );
...
export const createGame1 = (board: Cell[][], difficulty: NumberMap): Cell[][] => {
...
export const createGame = (state: GameState) => produce(state, (draft: GameState) => {
  ...
  // Object.values(numberMap).forEach((number: NumberMap) => {
